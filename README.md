...
Setup takes about 20 minutes. Configure your Notion database schema, create Supabase buckets, add environment variables, and drop the library files into your project.

## Claude Code Skill

This repo includes a `SKILL.md` — a [Claude Code](https://claude.ai/code) skill. Drop it into your project's `.claude/skills/notion-supabase-cms/` folder to scaffold the full setup automatically:

```bash
mkdir -p .claude/skills/notion-supabase-cms
curl -o .claude/skills/notion-supabase-cms/SKILL.md \
  https://raw.githubusercontent.com/funkuki/notion-supabase-cms/main/SKILL.md
```

Then run `/notion-supabase-cms` inside Claude Code to begin setup.
